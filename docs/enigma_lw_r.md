# Container `enigma_lw_r.sif`

Specific information about the `enigma_lw_r.sif` file goes here.
Modify as needed.

You may use the ``enigma_lw_r.sif`` container file to familiarize yourself with [Singularity](<https://sylabs.io/docs/>),
...
* Download ``enigma_lw_r.sif`` from [here](https://github.com/precimed/enigma_lw_r/tree/main/containers)
* Import these files to your secure HPC environment
* Run ``singularity exec --no-home enigma_lw_r.sif python3 --help``, to validate that you can run singularity. This command is expected to produce the standard python help message, starting like this:
...
1. The command
   ...
   ```
   singularity exec --home $PWD:/home enigma_lw_r.sif python -c "import os; print(f'Hello World from {os.getcwd()}')"
   ```
...
2. Now, let's assume that instead of downloading the ``enigma_lw_r.sif`` container file you've cloned the entire github repo
   (``git clone git@github.com:precimed/enigma_lw_r.git``), have built and pushed the container, and have transfered it to your HPC environment.
   Then change your folder to the root of the ``enigma_lw_r`` repository, and run these commands:

   ```
   singularity exec --bind reference/:/ref:ro,out_dir:/out:rw singularity/enigma_lw_r.sif python ...
   ```
...
3. Run
   ...
   ```
   singularity shell --home $PWD:/home -B $(pwd)/data:/data enigma_lw_r.sif
   ``` 
...
* Run singularity container within SLURM job scheduler, by creating a ``enigma_lw_r_job.sh`` file (by adjusting the example below), and running ``sbatch enigma_lw_r_job.sh``:

  ```
  #!/bin/bash
  #SBATCH --job-name=enigma_lw_r
  #SBATCH --account=$PROJECT
  #SBATCH --time=00:10:00
  #SBATCH --cpus-per-task=1
  #SBATCH --mem-per-cpu=8000M
  module load singularity/3.7.1
  singularity exec --no-home enigma_lw_r.sif python -c "print('Hello World')"
  singularity exec --home $PWD:/home enigma_lw_r.sif python -c "print('Hello Moon')"
  ```

Please [let us know](https://github.com/precimed/enigma_lw_r/issues/new) if you face any problems.
